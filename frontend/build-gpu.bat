@echo off
REM VoxMinutes GPU-Accelerated Build Script for Windows
...
echo.
echo ========================================
echo   VoxMinutes GPU-Accelerated Build
echo ========================================
echo.
...
REM llama-cpp-sys-2 的 bindgen 需要 libclang；优先使用仓库内 .tooling/llvm（未设置时自动探测）
if not defined LIBCLANG_PATH (
    if exist "%~dp0..\.tooling\llvm\bin\libclang.dll" (
        for %%I in ("%~dp0..\.tooling\llvm\bin") do set "LIBCLANG_PATH=%%~fI"
        echo    LIBCLANG_PATH auto-detected: !LIBCLANG_PATH!
    ) else (
        echo ⚠️  LIBCLANG_PATH not set and .tooling\llvm not found - build may fail without LLVM
    )
)
...
echo.
echo ========================================
echo   VoxMinutes GPU Build Script - Help
echo ========================================
echo.
echo USAGE:
...
echo   VoxMinutes with optimal hardware acceleration features:
echo.
