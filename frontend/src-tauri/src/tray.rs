...
pub fn create_tray<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<()> {
    ...
    let tray_icon = tauri::image::Image::from_bytes(include_bytes!("../icons/app_icon.ico"))
        .expect("Failed to load app_icon.ico for tray");
...
/// Check if recording is allowed based on transcription model availability
async fn check_can_record<R: Runtime>(app: &AppHandle<R>) -> bool {
    let _ = app;
    // Onboarding has been removed — recording is always allowed
    true
}
