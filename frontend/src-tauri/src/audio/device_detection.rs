...
impl InputDeviceKind {
    ...
    fn detect_by_name(device_name: &str) -> Option<Self> {
        ...
        const TIER2_BLUETOOTH_PATTERNS: &[&str] = &[
            ...
            "蓝牙",             // Bluetooth (Simplified Chinese)
            "藍牙",             // Bluetooth (Traditional Chinese)
            "블루투스",         // Bluetooth (Korean)
            "ブルートゥース",   // Bluetooth (Japanese)
        ];
        ...
        const TIER3_BLUETOOTH_PATTERNS: &[&str] = &[
            ...
            "无线",             // Wireless (Simplified Chinese)
            "無線",             // Wireless (Traditional Chinese / Japanese)
            "무선",             // Wireless (Korean)
        ];
