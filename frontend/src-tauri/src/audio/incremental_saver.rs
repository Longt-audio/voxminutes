...
impl IncrementalAudioSaver {
    ...
    async fn merge_checkpoints(&self, output: &PathBuf) -> Result<()> {
        ...
        for i in 0..self.checkpoint_count {
            ...
            // Use short path for FFmpeg to avoid non-ASCII path issues on Windows.
            let short_path = to_short_path_string(&checkpoint_path.canonicalize()?);
        ...
        // The final output file may not exist yet; short-path its parent.
        ...
        command.args(&[
            ...
            "-i", &to_short_path_string(&list_file),
...
pub async fn recover_audio_from_checkpoints(
    ...
    for entry in &checkpoint_files {
        ...
        let short_path = to_short_path_string(&path).replace('\'', "'\\''");
    ...
    let output_path_str = match (output_path.parent(), output_path.file_name()) {
        (Some(parent), Some(name)) => to_short_path(parent).join(name),
        _ => output_path.clone(),
    }
    .to_string_lossy()
    ...
    command.args(&[
        "-f", "concat",
        "-safe", "0",
        "-i", &to_short_path_string(&concat_file_path),
        "-c", "copy",
