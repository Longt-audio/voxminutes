...
/// Adaptive Automatic Gain Control (AGC) with noise floor gating
///
/// Boosts quiet speech before feeding into noise reduction and ASR,
/// while avoiding amplifying pure noise. Key properties:
/// - Noise floor estimation: gain is suppressed when signal is below noise threshold
/// - Smooth transitions: slow attack (50ms), slow release (300ms) for natural sound
/// - Max gain cap: +15 dB (5.6x) to prevent extreme amplification
/// - Target RMS: 0.12 (matches ~-18 dBFS, optimal for downstream models)
/// - Stateful: tracks running RMS across chunks for consistent leveling
...
impl AgcProcessor {
    pub fn new(_sample_rate: u32) -> Self {
        ...
        let attack_coeff = (-2.0f32 * std::f32::consts::PI * attack_ms / 1000.0).exp();
        let release_coeff = (-2.0f32 * std::f32::consts::PI * release_ms / 1000.0).exp();
    ...
    pub fn process(&mut self, samples: &[f32]) -> Vec<f32> {
        ...
        self.rms_smoother = self.rms_alpha * self.rms_smoother + (1.0 - self.rms_alpha) * rms;

        let desired_gain = if self.rms_smoother > self.noise_floor {
            (self.target_rms / self.rms_smoother.max(0.0001)).min(self.max_gain)
        } else {
            1.0
        };
        ...
        samples
            .iter()
            .map(|&x| (x * self.current_gain).clamp(-0.98, 0.98))
            .collect()
    }
}

/// Sherpa-ONNX GTCRN speech enhancement wrapper
///
/// Uses sherpa-onnx's built-in OfflineSpeechDenoiser with GTCRN model
/// to perform deep learning based noise suppression. Falls back gracefully
/// if the model file is not available.
///
/// GTCRN provides superior denoising compared to RNNoise, especially for
/// non-stationary noise environments (cafe, street, wind, etc.).
pub struct GtcrnDenoiser {
    denoiser: Option<sherpa_onnx::OfflineSpeechDenoiser>,
    expected_sample_rate: i32,
}

impl GtcrnDenoiser {
    pub fn new(model_path: &str, num_threads: i32) -> Self {
        use std::path::Path;

        if !Path::new(model_path).exists() {
            warn!(
                "GTCRN model not found at '{}' - speech enhancement will use RNNoise fallback",
                model_path
            );
            return Self {
                denoiser: None,
        ...
        let mut config = sherpa_onnx::OfflineSpeechDenoiserConfig::default();
        config.model.gtcrn.model = Some(model_path.to_string());
        config.model.num_threads = num_threads;
        config.model.provider = Some("cpu".to_string());

        match sherpa_onnx::OfflineSpeechDenoiser::create(&config) {
            Some(denoiser) => {
                let sr = denoiser.sample_rate();
                info!(
                    "GTCRN speech enhancement initialized (model: {}, expected sr: {} Hz)",
                    model_path, sr
                );
                Self {
            ...
            None => {
                warn!(
                    "Failed to create GTCRN denoiser from '{}' - falling back to RNNoise",
                    model_path
                );
                Self {
                    denoiser: None,
    ...
    /// Create an empty/unavailable denoiser (for when audio enhancement is disabled)
    pub fn empty() -> Self {
        Self {
            denoiser: None,
            expected_sample_rate: 16000,
    ...
    pub fn process(&self, samples: &[f32], sample_rate: i32) -> Vec<f32> {
        if let Some(ref denoiser) = self.denoiser {
            let result = denoiser.run(samples, sample_rate);
            result.samples
        } else {
            samples.to_vec()
        }
