...
/// Probe audio file metadata to get duration without decoding samples.
/// This is much faster than full decode and is used for progress estimation.
pub fn probe_audio_duration(path: &Path) -> Result<f64> {
    ...
    debug!("Probed audio duration: {:.2}s", duration_seconds);
    Ok(duration_seconds)
}
...
fn convert_to_wav_with_ffmpeg(
    ...
    let input_str = to_short_path_string(input_path);
    let output_str = to_short_path_string(&temp_path);
