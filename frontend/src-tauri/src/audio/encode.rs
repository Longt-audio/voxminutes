...
use crate::win_short_path::to_short_path;
use std::io::Write;
use std::sync::Arc;
use std::{
    path::PathBuf,
...
pub fn encode_single_audio(
    ...
    // The output file may not exist yet, so short-path its parent directory
    // and append the filename; this keeps non-ASCII install paths working.
    let output_arg = match (output_path.parent(), output_path.file_name()) {
